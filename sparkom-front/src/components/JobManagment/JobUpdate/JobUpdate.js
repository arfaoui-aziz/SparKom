...
import { selectJob, selectSelectedJob, setErrors, updateJob } from '../../../store/slices/jobs';
...
function JobUpdate() {
    ...
    useEffect( async () => {
      ...
      const [resultat, error] = await queryApi("company/showcompanies");
      if (err) {
        dispatch(setErrors(error));
        } else {
        dispatch(populateCompanies(resultat));
        }
    ...
    const formik = useFormik({
      ...
      onSubmit: async (values) => {
        ...
        setShowLoader(true);
        const [res, err] = await queryApi("job/updateJob/"+ job._id, values, "PATCH", false);
        if (err) {
          setShowLoader(false);
          setErrors({
            visible: true,
